...
def train_one_epoch(meant, stdt, meanb, stdb, epoch, model, optimizer, train_pg_graphs, batch_size, pct_close):# device
    ...
    for step, batch_graphs in enumerate(trainloader):
        ...
        rse = pow((batch_scores - batch_graphs.y),2) / pow((batch_graphs.y-meant),2)
    ...
    current_train_state = {
        ...
        'loss_train_rse': epoch_train_rse
    } 
    return current_train_state, optimizer

def evaluate_per_epoch(meant, stdt, meanb, stdb, epoch, model, eval_pg_graphs, batch_size, pct_close): #device
    ...
    with torch.no_grad():
        ...
        epoch_eval_rse /= (step+1)

        print('Epoch: {:04d}'.format(epoch),
                'loss_eval: {:.4f}'.format(epoch_eval_loss),
                'loss_rse: {:.4f}'.format(epoch_eval_rse),
                'time: {:.4f}s'.format(time.time() - start_time_training)
        )
...
def test_per_epoch(meant, stdt, meanb, stdb, epoch, model, eval_pg_graphs, batch_size, pct_close): #device
    ...
    with torch.no_grad():
        ...
        print('Epoch: {:04d}'.format(epoch),
                ...
                'rse_test: {:.4f}'.format(epoch_eval_rse),
                'time: {:.4f}s'.format(time.time() - start_time_training)
        )

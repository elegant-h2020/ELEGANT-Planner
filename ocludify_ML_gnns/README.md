...
1. [main_kfold_Nospatial_scaled.py](https://github.com/elegant-h2020/ELEGANT-Planner/blob/ML-GNNs/ocludify_ML_gnns/results/main_kfold_Nospatial_scaled.py) <br>
   ...
   **Argument List:** <br>
   ['main_kfold_Nospatial_scaled.py', 'gpu', 'team2', '--target_is', 'devicetransfer', 'team2/', 'team2/kfolder_gpu', 'team2/testfolder_gpu', 'team2/scaled_testfolder_gpu.csv', '--epochs', '100', '--batch_size', '32', 'experiments/', '--dataset_name', 'fromCummins', '--scale_type', 'minmax'] 

2. [GTM_kfold_nwrap_nospatial.py](https://github.com/elegant-h2020/ELEGANT-Planner/blob/ML-GNNs/ocludify_ML_gnns/results/GTM_kfold_nwrap_nospatial.py) <br>
   - This script contains the **class of Graph Transformer Model**. An important thing to be noted is that node embeddings are not calculated inside the ```class GTModel(nn.Module)```. Instead, we calculate them using the [NodeFeatureEncoder.py](https://github.com/elegant-h2020/ELEGANT-Planner/blob/ML-GNNs/ocludify_ML_gnns/results/NodeFeatureEncoder.py) and then pass them into our GNN model directly. The same applies to the edge embeddings by using [EdgeFeatureEncoder.py](https://github.com/elegant-h2020/ELEGANT-Planner/blob/ML-GNNs/ocludify_ML_gnns/results/EdgeFeatureEncoder.py).

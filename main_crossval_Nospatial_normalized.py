...
#from GTM_crossval_nowrap_nospatial_new import GTModel  
...
if __name__ == '__main__':    
    ...
    if (args.feature_scale==True) and (args.scale_type == 'standardization'):
    ...
    elif (args.feature_scale==True):
        print('minmax scaler')
        dataoutputs = kernels_filtered['OutputBytes'].values
        dataoutputs, Max, Min  = feature_scale(dataoutputs, args.scale_type)
        kernels_filtered['OutputBytes'] = dataoutputs

        dataexectime = kernels_filtered['End-to-End Time'].values
        dataexectime, Max, Min = feature_scale(dataexectime, args.scale_type)
        kernels_filtered['End-to-End Time'] = dataexectime
            ...
            for num_layers in [1]:#, 2]:#, 3, 5]:
                ...
                for exp in range(args.num_exps):
                    ...
                    if args.scale_type == 'standardization':
                        ...
                        test_state = test_per_epoch_minmax(Max, Min, epoch, model, test_pg_graphs, args.batch_size, args.pct_close)
                        whole_exp_states.append(test_state)
